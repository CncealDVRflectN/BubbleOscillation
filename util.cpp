...
{
    return calcGasPressure(value.derivR, time) - calcAcousticWavePressure(time) - 2.0 * SURFACE_TENSION_COEF / DENSITY -
           4.0 * VISCOSITY * value.secondDerivR / value.derivR;
}

double calcOmega(const CalcUnit& value, double time)
{
    return 2.0 * SURFACE_TENSION_COEF * value.secondDerivR / (value.derivR * value.derivR) -
           3.0 * SPECIFIC_HEAT_COEF * calcGasPressure(value.derivR, time) * value.secondDerivR / value.derivR +
           4.0 * VISCOSITY * value.secondDerivR * value.secondDerivR / (value.derivR * value.derivR) +
