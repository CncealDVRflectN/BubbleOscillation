...
void calcDerivativesKellerMiksis(double time, const CalcUnit* value, CalcUnit* result)
{
    ...
    result->secondDerivR = ((1 + value->secondDerivR / SOUND_VELOCITY) * (calcBoundaryPressure(value, time) - PRESSURE_DEFAULT) / DENSITY -
            1.5 * value->secondDerivR * value->secondDerivR * (1 - value->secondDerivR / (3 * SOUND_VELOCITY)) +
            value->derivR * calcOmega(value, time) / (SOUND_VELOCITY * DENSITY)) /
            (value->derivR * (1 - value->secondDerivR / SOUND_VELOCITY) + 4 * VISCOSITY / (SOUND_VELOCITY * DENSITY));
}
...
void calcKellerMiksisEuler()
{
    ...
    fopen_s(&resultOutput, KELLER_MIKSIS_EULER_OUTPUT, "w");
...
void calcKellerMiksisRungeKutta()
{
    ...
    fopen_s(&resultOutput, KELLER_MIKSIS_RUNGE_KUTTA_OUTPUT, "w");
